...
def get_models():
    ...
    models = dict()
    models["SVM (BoW)"]=SGDClassifier(loss='hinge',max_iter=500,random_state=42)
    models["Random Forest (BoW)"]=RandomForestClassifier()
    steps_SVM = [('svd', TruncatedSVD(2000)), ('classification', SGDClassifier(loss='hinge',max_iter=500,random_state=42))]
    models["SVM (SVD)"]=Pipeline(steps=steps_SVM)
    steps_RF = [('svd', TruncatedSVD(300)), ('classification', RandomForestClassifier())]
    models["Random Forest (SVD)"]=Pipeline(steps=steps_RF)
...
def report_and_predictions(model_name, model , X_train, y_train, X_test, y_test , X_test_unlabeled):
    ...
    results.to_csv('testSet_categories_'+model_name+'.csv', index=False, header=True)
...
df = pd.DataFrame(X_train[0].T.todense(), index=tfidf.get_feature_names(), columns=["TF-IDF"])
